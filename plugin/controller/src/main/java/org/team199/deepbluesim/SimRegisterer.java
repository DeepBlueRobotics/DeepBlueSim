...
/**
 * Performs automatic registration of callbacks detecting both the initialization of new devices as well as data callbacks for devices such as Motors, Gyros, etc.
...
public class SimRegisterer {
    ...
    static {
        ...
        // Register Initialized Callbacks for PWM Devices
        CALLBACKS.add(PWMSim.registerStaticInitializedCallback((name, isInitialized) -> {
            if(isInitialized) {
                callback("PWM", name);
            }
        }, true));
        // Register Initialized Callbacks for Encoder Devices
    ...
    // Initialize SimRegisterer. This method exists to ensure that the static block is called
    ...
    // This is especially important for SimDevice's because their initialized callbacks can be notified before their values have been created
    // Queuing also ensures that callbacks (which are usually executed asynchronously) are processed synchronously with the rest of the robot code
    ...
    private static void callback(String deviceName) {
        if(deviceName.startsWith("Talon") || deviceName.startsWith("Victor") || deviceName.startsWith("SparkMax")) {
            // If a new Talon or Victor has been initialized, attempt to link it to a Webots Motor
    ...
    private static void callback(String type, String port) {
        if(type.equals("PWM")) {
            // If a new PWM device has been initialized, attempt to link it to a Webots motor
            ...
            // If a new Encoder device has been initialized, attempt to link it to a Webots encoder
            // Register a speed callback on this device
